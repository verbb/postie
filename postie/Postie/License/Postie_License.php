...
class Postie_License
{
    ...
    public function __construct($endpoint, $product, $productVersion, $licenseKey = '')
    {
        ...
        $this->_model = new Postie_LicenseModel($attributes);
        ...
        try {
            ...
            if (!craft()->cache->get($this->etConnectFailureKey)) {
                ...
                if ($response->isSuccessful()) {
                    ...
                    $etModel = craft()->postie_license->decodeEtModel($response->getBody());

                    if ($etModel) {
                        return $etModel;
                    } else {
                        PostiePlugin::log('Error in calling ' . $this->_endpoint . ' Response: ' . $response->getBody(), LogLevel::Warning);
            ...
            PostiePlugin::log('Error in ' . __METHOD__ . '. Message: ' . $e->getMessage(), LogLevel::Error);
