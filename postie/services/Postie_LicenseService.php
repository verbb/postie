...
use Postie\License\Postie_License;

class Postie_LicenseService extends BaseApplicationComponent
{
    // Properties
    // =========================================================================
    ...
    private $plugin;
    private $pingStateKey = 'postiePhonedHome';
    private $pingCacheTime = 86400;
    private $pluginHandle = 'Postie';
    ...
    {
        $this->plugin = craft()->plugins->getPlugin('postie');
    ...
    public function ping()
    {
        if (craft()->request->isCpRequest() && !craft()->cache->get($this->pingStateKey)) {
            $et = new Postie_License(static::Ping, $this->pluginHandle, $this->pluginVersion, $this->licenseKey);
    ...
    {
        return craft()->plugins->getPluginLicenseKey('Postie');
    }

    public function setLicenseKey($licenseKey)
    {
        craft()->plugins->setPluginLicenseKey('Postie', $licenseKey);
        $this->licenseKey = $licenseKey;
    }

    public function getLicenseKeyStatus()
    {
        return craft()->plugins->getPluginLicenseKeyStatus('Postie');
    }

    public function setLicenseKeyStatus($licenseKeyStatus)
    {
        craft()->plugins->setPluginLicenseKeyStatus('Postie', $licenseKeyStatus);
    }

    public function getLicenseInfo()
    {
        $et = new Postie_License(static::GetLicenseInfo, $this->pluginHandle, $this->pluginVersion, $this->licenseKey);
    ...
    public function decodeEtModel($attributes)
    {
        if ($attributes) {
            ...
            if (is_array($attributes)) {
                $etModel = new Postie_LicenseModel($attributes);
    ...
    {
        $et = new Postie_License(static::UnregisterPlugin, $this->pluginHandle, $this->pluginVersion, $this->licenseKey);
        $etResponse = $et->phoneHome(true);
    ...
    {
        $et = new Postie_License(static::TransferPlugin, $this->pluginHandle, $this->pluginVersion, $this->licenseKey);
        $etResponse = $et->phoneHome(true);
    ...
    {
        $et = new Postie_License(static::RegisterPlugin, $this->pluginHandle, $this->pluginVersion, $licenseKey);
        $etResponse = $et->phoneHome(true);
    ...
    private function _handleEtResponse($etResponse)
    {
        if (!empty($etResponse->data['success'])) {
            ...
            if (!empty($etResponse->errors)) {
                ...
                PostiePlugin::log('License error: ' . $etResponse->errors[0], LogLevel::Error, true);
            } else {
                return false;
            }
        }
